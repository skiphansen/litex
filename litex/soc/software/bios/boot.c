...
unsigned int LoadGzipedData(unsigned char *pSrc,unsigned char *pDst,unsigned int Len);
...
static int copy_image_from_flash_to_ram(unsigned int flash_address, unsigned int ram_address)
{
   ...
   volatile unsigned int *pSrc = (volatile unsigned int *) flash_address;
   unsigned char *base_address;
   unsigned int *pDst = (unsigned int *) ram_address;
   unsigned int crc;
   unsigned int got_crc;
   int Ret = 0;
   ...
   do {
      ...
      base_address = (unsigned char *) pSrc;
      if(base_address[0] == 0x1f && base_address[1] == 0x8b) {
      // gziped data
         printf("Decompressing %d bytes from 0x%08x to 0x%08x...", length, flash_address, ram_address);
         if((length = LoadGzipedData(base_address,(unsigned char *)pDst,length)) <= 0) {
         // Decompression failed
            printf("Decompress Decompressoin faiCopying %d bytes from 0x%08x to 0x%08x...\n", length, flash_address, ram_address);
            break;
         }
         else {
         ...
         printf("Copying %d bytes from 0x%08x to 0x%08x...\n", length, flash_address, ram_address);
         while(Bytes2Copy >= sizeof(int)) {
            *pDst++ = *pSrc++;
            Bytes2Copy -= 4;
         }

         if(Bytes2Copy > 0) {
            memcpy(pDst,(void *) pSrc,Bytes2Copy);
         }
         printf("Checking CRC...");
         got_crc = crc32((unsigned char *)ram_address, length);
         if(crc != got_crc) {
            printf("\nCRC failed (expected %08x, got %08x)\n", crc, got_crc);
            break;
         }
         printf("PASSED\n");
      }
...
unsigned char *gLastIn;

int read_cb(struct uzlib_uncomp *d);
int read_cb(struct uzlib_uncomp *d)
{
   static int ByteCount;
   static unsigned int IntValue;
   static const unsigned char *cp;
   int BytesLeft;

   if(ByteCount-- <= 0) {
      BytesLeft = gLastIn - d->source;
      if(BytesLeft >= 4) {
         IntValue = *((unsigned int *) d->source);
         cp = (unsigned char *) &IntValue;
         d->source += 4;
         ByteCount = 3;
      }
      else if(BytesLeft > 0) {
         ByteCount = BytesLeft - 1;
         cp = d->source;
      }
      else {
         return -1;
...
// get decompressed length
   ...
   dlen =            pSrc[Len - 1];
   dlen = 256*dlen + pSrc[Len - 2];
   dlen = 256*dlen + pSrc[Len - 3];
   dlen = 256*dlen + pSrc[Len - 4];

   /* there can be mismatch between length in the trailer and actual
      pSrc stream; to avoid buffer overruns on overlong streams, reserve
      one extra byte */
   dlen++;
    ...
    d.source = pSrc;
    d.dest_start = d.dest = pDst;
    d.dest_limit = d.dest + dlen;
    d.source_limit = pSrc + Len - 4;
    d.source_read_cb = NULL;

    do {
       Ret = uzlib_gzip_parse_header(&d);
       if (Ret != TINF_OK) {
